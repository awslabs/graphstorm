...
GraphStorm has a variaty of built-in model components that can be easily combined to form model variants, e.g., turning an RGCN model into an HGT model, in the Command Line Interfaces(CLIs) by modifying :ref:`configurations<_configurations-run>` in the YAML file. Such built-in model variants can also be created by using GraphStorm APIs as demonstrated in `Notebook 3: Use GraphStorm APIs for Creating Built-in Model Variants <https://github.com/awslabs/graphstorm/blob/main/docs/source/api/notebooks/Notebook_3_Model_Variants.ipynb>`_.
