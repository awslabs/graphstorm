...
Users can run the ``gconstruct.construct_graph`` command by following the template below.
...
This template indicates the three required command arguments, i.e., a JSON file containing graph construction configurations, the directory for outputs, and a string as a name given to the constructed graph. The ``--num-parts`` is an important argument because it not only determines how many partitions to be constructed, also determines how many machines to be used in the GraphStorm model training and inference.
...
* **-\-num-parts**: an integer value that specifies the number of graph partitions to produce. This is only valid if the output format is ``DistDGL``. Default is ``1``.
