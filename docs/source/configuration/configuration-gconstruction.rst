...
* **Numerical Bucket transformation** normalizes numerical input features with buckets. Bucket feature transformation normalizes numerical input features with input buckets, where the input value will be divided into one of the buckets. The name field in the feature transformation dictionary is bucket. All input feature column data are categorized into respective buckets using this method, aiding in the normalization of input features. The bucket attribute is calculated based on the count number and the range of the bucket, and each bucket will have same length. Any input data lower than the minimum value will be assigned to the first bucket, and any input data exceeding the maximum value will be assigned to the last bucket. For example, with bucket=`[10,20,30]`, input data `1` will fall into the bucket `[10, 20]`, input data `11` will be mapped to `[10, 20]`, input data `21` will be mapped to `[20, 30]`, input data `31` will be mapped to `[20, 30]`. Finally we use one-hot-encoding to encode the feature for each numerical bucket. If user wants to make numeric values fall into more than one bucket, it is preferred to use the `slide_window_size`: `"slide_window_size": s` , where `s` is a number. Then each value `v` will be transformed into a range from `v - s/2` through `v + s/2` , and assigns the value `v` to every bucket that the range covers, and do the one-hot-encoding just as before.
