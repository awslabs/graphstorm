...
* **Floating MAX_MIN transformation** normalizes floating point input features with $val = (val-min)/(max-min)$, where $val$ is the feature value, $max$ is the maximum number in the feature and $min$ is the minimum number in the feature. The ``name`` field in the feature transformation dictionary is ``float_max_min``. The dict can contains two optional fields. ``max_bound`` specifies the maximum floating point value allowed in the feature. Any number larger than ``max_bound`` will be set to ``max_bound``. ``min_bound`` specifies the minimum floating point value allowed in the feature. Any number smaller than ``min_bound`` will be set to ``min_bound``.
