...
GraphStorm Environment Configurations
-------------------------------------
GraphStorm provides a set of parameters to control the execution environment.
...
- **backend**: PyTorch distributed backend, the suggested backend is gloo. Support backends include gloo and nccl
    ...
    - Default value: ``gloo``
- **ip_config**: (Required) Path to a file containing IPs of instances in a distributed training/inference cluster. In the ip config file, each line stores one IP. HINT: Use absolute path to avoid any path related problems.
    - Yaml: ``ip_config: example/ip_list.txt``
    - Argument: ``--ip-config example/ip_list.txt``
    - Default value: This parameter must be provided by user.
- **part_config**: (Required) Path to a file containing graph partition configuration. The graph partition is generated by GraphStorm Partition tools. HINT: Use absolute path to avoid any path related problems.
    - Yaml: ``part_config: example-graph/example.json``
    - Argument: ``--part-config example-graph/example.json``
    - Default value: This parameter must be provided by user.
...
GraphStorm Model Configurations
--------------------------------
...
Built-in Model Specific Config
................................
RGCN
`````
...
GraphStorm Model Save/Restore Configurations
---------------------------------------------
...
GraphStorm Model Evaluation Configurations
--------------------------------------------
...
GraphStorm Graph Partition Arguments
-------------------------------------------
Argument for Node/Edge Classification/Regress partition codes: `partition_graph.py <https://github.com/awslabs/graphstorm/blob/main/tools/partition_graph.py>`_
...
- **--num_parts**: integer value that specifies partitions the DGL graph to be split, required. Remember this number because we will need to set it in the model training step.
- **--output**: (Required) the folder path that partitioned DGL graph will be saved.

Arguments for Link Prediction partition codes: `partition_graph_lp.py <https://github.com/awslabs/graphstorm/blob/main/tools/partition_graph_lp.py>`_

- **--dataset**: (Required) the graph name defined for the saved DGL graph file.
- **--filepath**: (Required) the file path of the saved DGL graph file.
- **--target_etypes**: the canonical edge type for making prediction, required for link prediction. The GSF supports one predict edge type only. The format is ``src_ntype,etype,dst_ntype``, e.g., `author,write,paper`.
- **--train_pct**: a float value (\>0. and \<1.) with default value ``0.8``. If you want the partition script to split nodes/edges for training/validation/test sets, you can set this value to control the percentage of nodes/edges for training.
- **--val_pct**: a float value (\>0. and \<1.) with default value ``0.1``. You can set this value to control the percentage of nodes/edges for validation. 
