...
* ``task_type``: (**Required**) specifies the task defined on the nodes or edges. Currently, its value can be ``classification``, ``regression``, ``link_prediction``, and ``reconstruct_node_feat``.
