    ...
    for ntype, feats in fname_dict.items():
        for feat in feats:
            ...
            for path in (os.path.join(folder, name) for name in sorted( \
                ...
                nfeat = dgl.data.utils.load_tensors(f'{path}/node_feat.dgl')
                ...
                print("ntype", node_feats_data[feat].shape)
                num_parts += 1

            del nfeat
            gc.collect()
            whole_feat_tensor =  node_feats_data[feat]#torch.concat(tuple(t[feat] for t in node_feats_data), dim=0)
            metadata[feat] = {'shape': list(whole_feat_tensor.shape),
                              'dtype': str(whole_feat_tensor.dtype)}
            print(metadata[feat], num_parts)
            ...
            print("created wholegraph for", feat, "part", part_num)
