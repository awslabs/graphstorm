...
def wholegraph_processing(whole_feat_tensor, metadata, local_comm, feat, wg_folder, num_parts):
    ...
    if not use_low_mem:
        ...
        for type_name, feats in fname_dict.items():
            for feat in feats:
                ...
                for t in feats_data:
                    del t[feat]
                wholegraph_processing(whole_feat_tensor, metadata, local_comm, feat,
                                      wg_folder, num_parts)
            ...
            for feat in feats:
                ...
                for path in (os.path.join(folder, name) for name in part_files):
                    ...
                    if node_feats_data[feat] is None:
                        ...
                        node_feats_data[feat] = torch.concat((node_feats_data[feat], nfeat[feat]), dim=0)
                    num_parts += 1
                del nfeat
                gc.collect()
                wholegraph_processing(node_feats_data[feat], metadata, local_comm, feat,
                                      wg_folder, num_parts)
...
def trim_feat_files(trimmed_feats, folder, file_name):
    ...
    for part in range(num_parts):
        dgl.data.utils.save_tensors(os.path.join(folder, f'part{part}', 'new_' + file_name),
                                    trimmed_feats[part])
    # swap 'node_feat.dgl' files
    for part in range(num_parts):
        os.rename(os.path.join(folder, f'part{part}', file_name),
                  os.path.join(folder, f'part{part}', file_name + '.bak'))
        os.rename(os.path.join(folder, f'part{part}', 'new_' + file_name),
...
def main(folder, node_feat_names, edge_feat_names, use_low_mem=False):
    ...
    if node_feat_names:
        ...
        trimmed_feats = convert_feat_to_wholegraph(fname_dict, "node_feat.dgl", metadata,
                                                   folder, use_low_mem)
    ...
    if edge_feat_names:
        ...
        trimmed_feats = convert_feat_to_wholegraph(fname_dict, "edge_feat.dgl", metadata,
                                                   folder)
