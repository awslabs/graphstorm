...
class NodeIDShuffler():
    """ Shuffle node ids into the original node ids according to node_id_mappings
        ...
        rank: int
            Rank of the current process in a distributed environment.
    """
    def __init__(self, g, node_id_mapping_file, rank):
        self._g = g
        assert node_id_mapping_file is not None \
            and os.path.exists(node_id_mapping_file), \
            f"{node_id_mapping_file} must exist."
        self._rank = rank
        self._node_id_mapping_file = node_id_mapping_file
        # We do not load all the id mapping in init.
        # We will do lazy loading.
    ...
    def _load_id_mapping(self, ntype):
        ...
        if self._rank == 0:
            id_mappings = th.load(self._node_id_mapping_file)
