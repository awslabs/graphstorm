...
class NodeIDShuffler():
    """ Shuffle node ids into the original node ids according to node_id_mappings
        ...
        node_id_mapping_file:
            ...
            graph partition algorithm.
    """
    def __init__(self, g, node_id_mapping_file):
        self._g = g
        assert node_id_mapping_file is not None \
            and os.path.exists(node_id_mapping_file), \
            f"{node_id_mapping_file} must exist."

        # _id_mapping_info stores node ids in distributed tensors.
        # ID mappings are not load in __init__ function
        # The mappings are loaded on-demand.
        self._id_mapping_info = {}
        # _id_mappings is used by rank0 to cache the node id mapping file.
        if get_rank() == 0:
            self._id_mappings = th.load(node_id_mapping_file)
    ...
    def _load_id_mapping(self, ntype):
        ...
        barrier()
        self._id_mapping_info[ntype] = id_mapping_info
    ...
    def shuffle_nids(self, ntype, nids):
        """ Shuffle node ids of nype into their original id space.
            ...
            nids: torch.Tensor
                Node ids.
        """
        if ntype not in self._id_mapping_info:
            # We need to load the id mapping of ntype into memory first
            self._load_id_mapping(ntype)
