...
class HeteroGraphConv(nn.Module):
    ...
    def _max_reduce_func(self, inputs, dim):
        return th.max(inputs, dim=dim)[0]

    def _min_reduce_func(self, inputs, dim):
        return th.min(inputs, dim=dim)[0]

    def _sum_reduce_func(self, inputs, dim):
        return th.sum(inputs, dim=dim)

    def _mean_reduce_func(self, inputs, dim):
        return th.mean(inputs, dim=dim)

    def _stack_agg_func(self, inputs, dsttype):  # pylint: disable=unused-argument
        if len(inputs) == 0:
            return None
        return th.stack(inputs, dim=1)

    def _agg_func(self, inputs, dsttype, fn):  # pylint: disable=unused-argument
        if len(inputs) == 0:
            return None
