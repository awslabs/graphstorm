...
class ParquetArrayParser(object):
    """Wrapper class for parquet read/write operations."""
    ...
    def read(self, path):
        ...
        # Spark ML feature processing produces single-column parquet files
        # where each row is a vector object
        if len(data_types) == 1 and isinstance(data_types[0], pyarrow.ListType):
            arr = np.array(table.to_pandas().iloc[:, 0].to_list())
            logging.debug(
                "Parquet data under %s converted from single vector "
                "per row to ndarray",
                path,
            )
        else:
        ...
        shape = tuple(literal_eval(shape.decode())) if shape else arr.shape
    ...
    def write(self, path, array, vector_rows=False):
        ...
        vector_rows: bool
            Flag to indicate that the write operation is a single vector
            write.
        """
