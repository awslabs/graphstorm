...
def main():
    ...
    if os.path.exists(launch_arguments_path):
        # Determine GSProcessing output config name from input arguments
        ...
        gsp_config_fpath = os.path.join(args.input_path, gsprocessing_config_fname)

        # Copy GSProcessing runtime-updated config to output path with new name so it can be used
        # during inference
        dst_config = os.path.join(output_path, 'dist_graph', GS_RUNTIME_GCONSTRUCT_FILENAME)
        ...
        shuffle_hard_negative_nids(
            ...
            graph_path=args.output_path)
    else:
        logging.warning("Could not find GSProcessing launch arguments file.")
