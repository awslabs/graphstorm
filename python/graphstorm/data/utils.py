...
from sklearn.preprocessing import LabelBinarizer # pylint: disable=import-error
from sklearn.preprocessing import MultiLabelBinarizer # pylint: disable=import-error
...
def load_raw_text(path, dname, range):
    config_path = "{}.pkl".format(dname)
    config_path = os.path.join(path, config_path)
...
def return_reverse_mappings(etypes,g):
    # these mappings are needed by the edge dataloader to exclude the reverse edges in the computational graph
    # here we assume that the reverse edge id has the same id as the original edge id. this is a result of the
    # add_reverse_edges() function of the data utils files.
    reverse_eids = {g.to_canonical_etype(etype): th.arange(g.number_of_edges(etype)) for etype in etypes}
    return reverse_eids
...
def adjust_eval_mapping_for_partition(original_idx, original_eval_idx, original_eval_text):
    '''
    This function corrects the mappings for the evaluation entity. It will search over the mapped idx where the original
    eval idx is contained and will returned the position and the corresponding text
    Returns
    -------
    ...
    xy, x_ind, y_ind =np.intersect1d(original_idx, original_eval_idx, assume_unique=True, return_indices=True)
    ...
    # original_node_idx_to_distributed_node_idx= { original_eval_idx : x_ind[i] for i in range(len(x_ind))}
...
def generated_train_valid_test_splits(g,train_pct,valid_pct,test_pct, use_non_selected_edges=False, seed=None):
    ...
    for etype in g.canonical_etypes:
        ...
        val_eids[etype] = edge_ids[int(train_pct*number_edges_etype):int((train_pct+valid_pct)*number_edges_etype)]
        ...
        print('Edge type : {}: |train|={}, |val|={}, |test|={}'.format(str(etype),len(train_eids[etype]),
                                                                       len(val_eids[etype]), len(test_eids[etype])))

    return train_eids, val_eids, test_eids, train_graph_mask_dic, val_graph_mask_dic, test_graph_mask_dic
